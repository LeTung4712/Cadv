...
double shortestPath(Graph graph, int start, int stop, int *path, int *numVertices)
{
    ...
    visit[start] = 1;
    //khởi tạo queue lưu trũ các đỉnh thăm
    Dllist ptr, queue, node;
    queue = new_dllist();
    dll_append(queue, new_jval_i(start));//cho đỉnh chọn làm gốc duyêt vào queue
    ...
    while (!dll_empty(queue))
    {
        ...
        u = jval_i(node->val);
        visit[u] = 1;//đánh dấu đỉnh đang duyệt là đã thăm
        dll_delete_node(node);
        if (u == stop)
            break;

        int output[1000];
        int n = outdegree(graph, u, output);//nhận các đỉnh ra từ u
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if (visit[v] == 0)//chưa thăm thì thêm vào queue
            {
                dll_append(queue, new_jval_i(v));
            }
        }
...
//tương tự như shortest
double longestPath(Graph graph, int start, int stop, int *path, int *numVertices)
{
    double distance[1000];
    int previous[1000], u, visit[1000];
    ...
    while (!dll_empty(queue))
    {
        ...
        u = jval_i(node->val);
        //visit[u] = 1; không cần xét đã thăm hay chưa
        dll_delete_node(node);
        //if (u == stop)
          //  break;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            dll_append(queue, new_jval_i(v)); //thêm vào queue luôn mà không cần xét đã thăm đỉnh hay chưa
...
int topo(Graph graph, int start, int *path)
{
    int j=0,u,visit[1000];
    ...
    while (!dll_empty(queue))
    {
        ...
        int output[1000];
        int n = indegree(graph, u, output);//nhận các đỉnh ra từ u
